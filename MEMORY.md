...
### 1.6 PhotoGallery deploys behind nginx-appeid at `/photogallery` ΓÇö prefix-aware end-to-end

- **Convention**: PhotoGallery deploys behind `nginx-appeid` at `/photogallery`. `ConfigurationSettings:BasePath` + `app.UseForwardedHeaders()` + `app.UsePathBase(basePath)` drive prefix awareness server-side; the FE ships `<base href="/photogallery/">` at build time via `ng build --base-href=/photogallery/` (the `npm run build:prod` script); nginx preserves the prefix and sets `X-Forwarded-Prefix: /photogallery`; the SignalR hub is reachable at `/photogallery/hubs/photo-progress` via a dedicated nginx `location` block; the legacy `sub_filter` HTML rewrite is retired. Future tenants on the same proxy follow the same four-part shape.
- **Citation**: `Documentation/Architecture/DESIGN_DECISIONS.md` (D018); `PhotoGallery/Program.cs` (lines ~140-155 `ForwardedHeadersOptions` with cleared `KnownProxies`/`KnownNetworks`, lines ~396-418 `UseForwardedHeaders()` → `UsePathBase(settings.BasePath)`); `Configuration/ConfigurationSettings.cs` (`BasePath` property); `PhotoGallery/appsettings.Trial.json` and `appsettings.Production.json` (`"BasePath": "/photogallery"`); `FE.PhotoGallery/package.json` (`build:prod`, `start:proxy` scripts); `nginx-appeid/Services/appeid/server-appeid.conf` (post-`nginx-appeid#9` shape). Stories: #168/#171/#169/#172/#173 + `nginx-appeid#9` of epic #159.
- **Reason**: The proxy hosts multiple tenants on one apex (`appeid.app/photogallery/`, `appeid.app/foundryvtt/`, ...). Without a documented paved road, every new tenant reinvents some combination of nginx-strip + `--base-href` + `UsePathBase`, usually inconsistently. The four-part pattern is the canonical Microsoft-recommended shape and makes `MapHub` / `MapControllers` prefix-aware with no per-endpoint code change.
- **Pitfall**: Three regressions to watch for. (1) Forgetting to clear `KnownProxies`/`KnownNetworks` on `ForwardedHeadersOptions` ΓÇö the middleware silently no-ops behind ACA ingress, the backend sees `http://` + no prefix, and OpenAPI/`LinkGenerator` URLs come out wrong. (2) Re-introducing `sub_filter '<base href="/">' '<base href="/photogallery/">'` in nginx ΓÇö it breaks on compressed responses, CDN edges, and any future `<base>` element layout change; always rely on build-time `--base-href` instead. (3) Deploy order: per D016, the BasePath-aware backend (#168) must reach `trial` **before** the nginx prefix-preserve change (`nginx-appeid#9`), or the proxy forwards `/photogallery/api/*` to a backend that 404s the prefix.
