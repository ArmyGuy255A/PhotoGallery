...
This is the orchestrator-side view of post-PR validation. The `pg-qa-quality-control` agent drives this flow; the `pg-playwright-tester` agent executes Steps 4 + 5.

> *→ Step 1 trigger — consult `pr-review-checklist` for review-gate criteria.*
> *→ Steps 4-5 execution — consult `playwright-testing-skill` (this repo) and `playwright-test-recipe` (plugin).*

### The 7 phases

1. **Trigger.** A PR has been opened or pushed. Manual handoff: developer (or `pg-project-manager`) invokes `pg-qa-quality-control`.
2. **Branch switch.** Fetch and switch to the PR head ref:
   ```pwsh
   $prNum = <pr-number>
   $head  = (gh pr view $prNum --json headRefName -q .headRefName)
   git fetch origin $head
   git switch $head
   ```
3. **Feature inventory.** Diff the PR against its base to identify user-visible changes:
   ```pwsh
   $base = (gh pr view $prNum --json baseRefName -q .baseRefName)
   git --no-pager diff "origin/$base...HEAD" --name-only
   ```
   Classify changes: FE components/pages, BE endpoints, auth flow changes, data-model changes.
4. **Coverage check.** For each user-visible change, look for a matching Playwright spec under `tests/e2e/`. If missing, hand off to `pg-playwright-tester` to author one (using `playwright-test-recipe` patterns and the project's existing page-object conventions).
5. **Run.** Execute the suite (or scoped subset for fast feedback):
   ```pwsh
   ...
   Pop-Location
   ```
   Capture: pass/fail count, failed-test names, `playwright-report/` path.
6. **Comment on PR.** Post a structured result to the PR:
   ```pwsh
   $body = @"
   ...
   **Base:** ``$base``
   ...
   ### Specs added
   - <list of new spec files, or "none — coverage already in place">
   ...
   - **<test name>** — <one-line reason>
     - Repro: ``npx playwright test <file> --grep "<title>"``
   
   _Generated by ``pg-qa-quality-control``_
   "@
   gh pr comment $prNum --body $body
   ```
7. **Hand back.** If failures → label PR `needs-fix`, request changes, assign back to author. If pass → label `qa-passed`, dismiss any prior `needs-fix`. Use `gh pr edit $prNum --add-label <label>`.

### Failure-handling rules

- **Flaky failures** (intermittent across re-runs) — file a `bug` issue with reproduction steps before labeling `needs-fix`. Don't paper over with retries.
- **Missing test infra** (test-token endpoint, KeyCloak realm) — hand off to `pg-aspnet-backend-dev` / `pg-devops-cicd`, NOT `needs-fix` on the PR author.
- **Spec authoring needed but blocked** (e.g., FE change is a refactor with no behavioral diff) — comment "no new spec needed; refactor verified by existing coverage" and proceed to label.

### What not to do

- Don't run validation on a PR you authored (conflict of interest — ask another contributor or use the agent in CI mode in future).
- Don't commit Playwright spec changes onto the PR branch unless explicitly approved by the PR author. Open a sibling PR or commit to a `u/<actor>/test/<scope>` branch and reference it in the comment.
- Don't leave a stale `needs-fix` label after the author has pushed a fix — re-run the validation first.
